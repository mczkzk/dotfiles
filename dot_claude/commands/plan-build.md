...
1. 🔍 **Find Plan Search**: 
   - If feature-name provided: Look for `.agent/plans/[feature-name]/search.md` file
   - If no feature-name: Search `.agent/plans/` directory for `*/search.md` files (excluding `archive/` subdirectory) and identify from context
...
2. ✅ **STRICT Completion Verification**: 
   ...
   - Run verification command: `grep '\- \[ \]' ".agent/plans/[feature-name]/search.md"` to confirm zero results
...
4. 📝 **Create Plan Document**: 
   - Generate `.agent/plans/[feature-name]/plan.md` using template below ONLY after verification and guidelines review complete
...
5. 🔍 **MANDATORY Section Completeness Verification**: 
   ...
   - Run verification command: `grep -c "^## " ".agent/plans/[feature-name]/plan.md"`
