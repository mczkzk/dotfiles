...
**切り取り幅は貼った先の表示幅を決める**ので、貼る場所の列幅を先に知っておく。retina で撮ると
PNG は見えていた大きさの 2 倍になるため、**PNG 幅が「列幅 × 2」を超えたら縮小されて読めなくなる**。
GitHub の PR 本文なら列幅は約 814px なので PNG は 1600px 程度が上限。超えるなら主張に関係ない列を
落とすか 2 枚に分ける。

貼るときは **`width` を自分で指定する**。GitHub はアップロード時に `width="<PNG 幅>"` を入れるので、
retina (`scale: device`) で撮った画像はそのままだと 2 倍で表示される。**PNG の寸法の半分**を渡すと
画面で見たとおりの 1:1 になる。`scale: css` で撮った画像は等倍なので PNG 幅のままでよい。
`getBoundingClientRect().width / (naturalWidth / 2)` が 1.0 になることで確かめられる。
