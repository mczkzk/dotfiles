...
**Group 2 — Rule & convention compliance** (matches diff against codified standards)
...
**Group 3 — Completeness & consistency** (stress-tests the diff for self-consistency)

| Agent | Type | Responsibility |
|-------|------|----------------|
| **E** | `symmetry-checker` | Find asymmetric validation / control-flow: a guard on "A set" asserts about B, but no reverse guard on "B set" about A (or an "A ⇔ B" combined guard) exists. Flags newly introduced invariant-violation shapes the diff accepts |
| **F** | `claim-verifier` | Verify strong claims in new/modified comments (idempotent, atomic, thread-safe, ensures X, prevents Y) against the actual code. Flags comments that lie |

**Group 4 — Impact & dependency analysis** (reads beyond the diff to trace ripple effects)

| Agent | Type | Responsibility |
|-------|------|----------------|
| **G** | `code-tracer` | Trace the impact radius of changed functions/modules: enumerate callers and callees, follow data flow through transformations, detect circular or broken dependency chains. Focus on non-obvious indirect effects the diff alone does not reveal |
...
| **I** | `reuse-finder` | New definitions that duplicate existing code in the codebase |

**Group 5 — Historical context** (leverages git history and prior PRs)
