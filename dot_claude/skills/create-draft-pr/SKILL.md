...
8. **Attach Screenshots** (only if step 3 found images worth attaching)
   ...
   - **The DevTools panel needs a different capture path, not a text substitute.** Playwright MCP and the Chrome DevTools MCP cannot screenshot it (`list_pages` shows only inspected pages), but the DevTools window *is* a CDP `page` target on a Chrome launched with `--remote-debugging-port` + `--auto-open-devtools-for-tabs`, so it can be captured with no extra dependencies. Follow the `e2e-verify` skill's `references/devtools-capture.md`. Only fall back to stating the data in text when that path is genuinely unavailable
   - **Annotate at the unit of comparison, and only when needed.** The test: name what the reviewer should conclude, then ask whether the bare image already forces it. A file list whose names and count *are* the finding needs no boxes. But when the finding is a contrast across N items, box **all N** — boxing only the odd one out reads as "this one is broken" instead of "compare these". Put labels in empty space, never over the subject or over a value that is itself the evidence. Crop every image to the region that carries the finding (~28px of margin) before deciding on boxes; see the `e2e-verify` skill's "Cropping and annotation" section for both
   ...
   - **Set the display width yourself; the upload's default is wrong for a retina capture.** The body column is about 814px and GitHub honours the `width` attribute as-is, so the upload's `width="<png width>"` shows a `scale: device` capture at 2x, which reads as blown up. Pass **half the PNG's dimensions** and it renders 1:1 with what you saw on screen, at retina sharpness. Anything wider than the column is scaled *down* instead, which is worse: a 2820px capture landed at 0.27x with its status column illegible. If half the PNG width still exceeds ~814, the crop is too wide — drop the columns that are not the finding, or split it into two images. Verify with `getBoundingClientRect().width / (naturalWidth / 2)` and expect 1.0
